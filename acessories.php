<!-- filepath: /d:/New folder (3)/accessories.php -->
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <div class="products-container">
        ...
        if (!$subcategory_id) {
            echo "<p>Invalid subcategory ID</p>";
            exit;
        }

        // Fetch accessories from the database
        $stmt = $conn->prepare("SELECT id, name, image FROM accessories WHERE subcategory_id = ?");
        $stmt->bind_param("i", $subcategory_id);
        $stmt->execute();
        $accessories_result = $stmt->get_result();

        if ($accessories_result->num_rows > 0) {
            while ($accessory = $accessories_result->fetch_assoc()) {
                echo '<div class="product-item">';
                echo '<a href="accessory_detail.php?accessory_id=' . htmlspecialchars($accessory["id"]) . '">';
                echo '<img src="admin/' . htmlspecialchars($accessory["image"]) . '" alt="' . htmlspecialchars($accessory["name"]) . '">';
                echo '<p>' . htmlspecialchars($accessory["name"]) . '</p>';
                echo '</a>';
                echo '</div>';
            }
        } else {
    ...
    <footer style="width: 100%; background-color: #f1f1f1; text-align: center; padding: 10px 0; box-shadow: 0 -1px 5px rgba(0,0,0,0.1); position: fixed; bottom: 0;">
